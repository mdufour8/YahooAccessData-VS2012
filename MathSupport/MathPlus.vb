...
#Region "Definition"
	...
	Public Module VectorList
		Public Function Create(Of T)(ByVal Length As Integer) As IList(Of T)
...
#Region "FilterStochasticBrownian"
		...
		Public Class FilterStochasticBrownianUltimate
			...
			Private MyFilterVolatilityForBrownianStatistic As FilterVolatilityYangZhang
...
#Region "FilterOBV"
		...
		Public Class FilterOBV
			...
			Private MyVolumeLast As Integer
			...
			Private MyFilterForOBV As IFilter

			Public Sub New(ByVal FilterRate As Integer)
				If FilterRate < 1 Then FilterRate = 1
				MyRate = FilterRate
				'MyFilterForOBV = New FilterLowPassExp(MyRate)
				MyFilterForOBV = New FilterLowPassPLL(MyRate)
				'by default
			...
			Public Sub New(ByVal FilterRate As Integer, ByVal VolumeLimitFactor As Double)
				Me.New(FilterRate)
				MyLimitFactorForVolume = VolumeLimitFactor
			End Sub
			...
			Public Sub New(ByVal PreFilterRate As Integer, ByVal FilterRate As Integer)
				...
				MyRatePreFilter = PreFilterRate
				If MyRatePreFilter > 1 Then
					MyPreFilterForPrice = New FilterLowPassExp(MyRatePreFilter)
					MyPreFilterForVolume = New FilterLowPassExp(MyRatePreFilter)
				End If
			End Sub

			Public Sub New(ByVal PreFilterRate As Integer, ByVal FilterRate As Integer, ByVal VolumeLimitFactor As Double)
				Me.New(PreFilterRate, FilterRate)
				MyLimitFactorForVolume = VolumeLimitFactor
			End Sub

			Public Function Filter(ByVal Volume As Integer, ByVal Direction As FilterRSI.SlopeDirection) As Double
				...
				Else
					ThisVolumeFiltered = MyFilterForVolumeLowPassExp.Filter(CDbl(Volume))
					'limit the volume 
				...
				If ThisVolumeValueFiltered > 0 Then
					...
					End Select
				End If
				Return MyFilterForOBV.Filter(MyOBVLast)
			End Function

			Public Function Filter(ByVal Price As Double, ByVal Volume As Integer, Optional ByVal Direction As FilterRSI.SlopeDirection = FilterRSI.SlopeDirection.NotSpecified) As Double
			...
			Public Function FilterPredictionNext(ByVal Price As Double, ByVal Volume As Integer) As Double
				...
				Else
					ThisPriceValueFiltered = MyPreFilterForPrice.FilterPredictionNext(Price)
					If ThisVolumePeakFiltered = 0 Then
						'do not filter volume of zero
						ThisVolumeValueFiltered = 0
					Else
						ThisVolumeValueFiltered = MyPreFilterForVolume.FilterPredictionNext(CDbl(ThisVolumePeakFiltered))
					End If
				End If
				If MyFilterForOBV.Count = 0 Then
					'OBV initialisation 
				...
				End If
				ThisPriceFilteredLast = ThisPriceValueFiltered
				Return MyFilterForOBV.FilterPredictionNext(ThisOBVLast)
			End Function
				...
				With PriceVol
					If .IsIntraDay Then
						If .Last >= .LastPrevious Then
							Return Me.Filter(CDbl(.LastWeighted), .VolPlus)
						Else
							Return Me.Filter(CDbl(.LastWeighted), .VolMinus)
						End If
					Else
						Return Me.Filter(CDbl(.LastWeighted), .Vol)
					End If
				End With
			End Function
			...
			Public Function FilterLast() As Double
				Return MyFilterForOBV.FilterLast
			End Function
			...
			Public Function VolumeLast() As Integer
				Return MyVolumeLast
			End Function
			...
			Public ReadOnly Property Count As Integer
				Get
					Return MyFilterForOBV.Count
				End Get
			End Property

			Public ReadOnly Property Max As Double
				Get
					Return MyFilterForOBV.Max
				End Get
			End Property

			Public ReadOnly Property Min As Double
				Get
					Return MyFilterForOBV.Min
				End Get
			End Property

			Public ReadOnly Property ToList() As IList(Of Double)
				Get
					Return MyFilterForOBV.ToList
				End Get
			End Property

			Public Function ToArray() As Double()
				Return MyFilterForOBV.ToArray
			End Function
			...
			Public Function ToArray(ByVal MinValueInitial As Double, ByVal MaxValueInitial As Double, ByVal ScaleToMinValue As Double, ByVal ScaleToMaxValue As Double) As Double()
				Return MyFilterForOBV.ToArray(MinValueInitial, MaxValueInitial, ScaleToMinValue, ScaleToMaxValue)
			End Function
