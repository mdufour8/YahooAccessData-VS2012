...
Namespace MathPlus.Filter
  ...
  Public Class FilterLowPassPLLPredict
    ...
    Public Sub New(ByVal NumberToPredict As Double, ByRef InputValue() As Double, ByVal FilterHead As IFilter)
      MyBase.New(NumberToPredict:=NumberToPredict, InputValue:=InputValue, FilterHead:=FilterHead, FilterBase:=New FilterLowPassPLL(FilterHead.Rate, IsPredictionEnabled:=False))
    End Sub

    Public Sub New(ByVal FilterRate As Double, ByVal NumberToPredict As Double, ByRef InputValue() As Double)
      MyBase.New(NumberToPredict:=NumberToPredict, InputValue:=InputValue, FilterHead:=New FilterLowPassPLL(FilterRate), FilterBase:=New FilterLowPassPLL(FilterRate, IsPredictionEnabled:=False))
    End Sub
  End Class
End Namespace