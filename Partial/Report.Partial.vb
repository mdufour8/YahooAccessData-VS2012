...
#Region "Maintenance Functions"
  ...
  End Function
	...
	''' <summary>
	''' Remove stock created by the local user only. The databse stock cannot be removed with this function
	''' </summary>
	''' <param name="StockSymbol">The symbol of the stock to remove.</param>
	''' <returns>True if the stock was successfully removed; otherwise, False.</returns>
	Public Function StockRemove(StockSymbol As String) As Boolean
		Dim ThisStock As YahooAccessData.Stock = Nothing
		Dim ThisSector As YahooAccessData.Sector = Nothing
		Dim ThisIndustry As YahooAccessData.Industry = Nothing

		'get rid of the empty string
		If String.IsNullOrEmpty(StockSymbol) Then Return False

		ThisStock = Me.Stocks.ToSearch.Find(StockSymbol)
		If ThisStock Is Nothing Then Return False

		If ThisStock IsNot Nothing Then
			If ThisStock.IsGeneratedStock = False Then
				'cannot remove a stock that is not generated by the user
				Return False
			End If
			With ThisStock
				.Sector?.Stocks?.Remove(ThisStock)
				.Industry?.Stocks.Remove(ThisStock)
				.Sector = Nothing
				.Industry = Nothing
			End With
		End If
		Me.Stocks?.Remove(ThisStock)
		Return True
	End Function
						...
						For Each ThisStockSymbol As IStockSymbol In ThisListOfStockSymbol
							...
							If ThisStock IsNot Nothing Then
								With ThisStock
									'	'keep most of the information except a few item
									.Name = ThisStockSymbol.Name
									.Exchange = ThisWebYahooStockDescriptor.ExchangeCode
