...
Public Module StockPriceLogGainExtensions
	...
	Public Function ToCumulativeLogGain(
		...
		For Each SourceItem In DataSource
			...
			'the value of zero should not affect the rest of calculation except for that point, and it is better than throwing
			...
			'reference to the last bar's Last price is the correct way to compute the log gain for the current bar,
			'since the log gain is defined as the log of the ratio of the current price to the previous price,
			'and the previous price is represented by the last bar's Last price. Using the last bar's Open, High,
			...
			Record.CheckPrice(SourceItem.AsIPriceVol)
			...
			'and low of different symbols usually do not occur at the same time.
			'
			'Directly adding HighGain or LowGain would implicitly assume that all symbols reached
			'their intraday extremes simultaneously, which would usually exaggerate the synthetic
			'price range.
			'
			'Instead, we combine the CloseGain normally, then estimate the High/Low range as an
			'excess gain relative to the CloseGain:
			'
			'   HighExcess = HighGain - CloseGain
			'   LowExcess  = LowGain  - CloseGain
			'
			'The synthetic High/Low gain is then estimated by applying a weighted average of those
			'excess ranges around the synthetic CloseGain.
			'
			'This is not a perfect intraday model, because some correlation between symbols still
			'exists, but it avoids the unrealistic amplification caused by adding uncorrelated peaks
			'as if they happened at the same time. So this code need to be revisited.

			ThisDataResultItem.Last = Measure.GainLog(SourceItem.Last, ThisSourceItemLast.Last, OnErrorReturn:=0.0) + ThisDataResultLast.Last
			ThisDataResultItem.Open = Measure.GainLog(SourceItem.Open, ThisSourceItemLast.Last, OnErrorReturn:=0.0) + ThisDataResultLast.Last
			ThisDataResultItem.High = Measure.GainLog(SourceItem.High, ThisSourceItemLast.Last, OnErrorReturn:=0.0) + ThisDataResultLast.Last
			ThisDataResultItem.Low = Measure.GainLog(SourceItem.Low, ThisSourceItemLast.Last, OnErrorReturn:=0.0) + ThisDataResultLast.Last
	...
	''' <summary>
	''' Converts a sequence of StockPriceVol objects to their log gain representation. Here the gains are not cumulated.
	''' This can be usuful when some form of time sequncing is needed to be applied to the data, for example when calculating 
	''' the gain of a stock when another sequqncing signal indicate when to buy, hold or sale the stock. The function will return a new list of StockPriceVol with the same number of items as the input, 
	''' but with the Open, High, Low, Last, OpenNext and LastPrevious fields adjusted to represent the log gain relative to the previous bar.
	''' </summary>
	''' <param name="source"></param>
	''' <returns></returns>
	<Extension>
	Public Function ToLogGain(
			source As IEnumerable(Of StockPriceVol)) As List(Of StockPriceVol)
		...
		For Each SourceItem In DataSource
			...
			'bad data has not too much impact on the overall analysis.
			'This is a pragmatic approach to handle data quality issues while still providing useful results.
			ThisDataResultItem.DataType = StockPriceDataType.LogReturn
			'.Open = Measure.GainLog(SourceItem.Open, ThisSourceItemLast.Open, OnErrorReturn:=0.0) + ThisDataResultLast.Open
			'.High = Measure.GainLog(SourceItem.High, ThisSourceItemLast.High, OnErrorReturn:=0.0) + ThisDataResultLast.High
			'.Low = Measure.GainLog(SourceItem.Low, ThisSourceItemLast.Low, OnErrorReturn:=0.0) + ThisDataResultLast.Low
			'.Last = Measure.GainLog(SourceItem.Last, ThisSourceItemLast.Last, OnErrorReturn:=0.0) + ThisDataResultLast.Last
			...
			ThisDataResultItem.Last = Measure.GainLog(SourceItem.Last, ThisSourceItemLast.Last, OnErrorReturn:=0.0)
			ThisDataResultItem.Open = Measure.GainLog(SourceItem.Open, ThisSourceItemLast.Last, OnErrorReturn:=0.0)
			ThisDataResultItem.High = Measure.GainLog(SourceItem.High, ThisSourceItemLast.Last, OnErrorReturn:=0.0)
			ThisDataResultItem.Low = Measure.GainLog(SourceItem.Low, ThisSourceItemLast.Last, OnErrorReturn:=0.0)

			'Open next is not yet known but by default we can set it to be the same as Last
			'until we see the next bar and can update it, this way we avoid having a gap
			'in the data and we will have a valid value for OpenNext for all bars,
			'even if it is not the exact value until we update it with the next bar's Open
			'in non cumulative log gain, the OpenNext value Is Not meaningful because it does Not represent a cumulative value, but we still 
			'set it to Last for consistency And to avoid nulls.
			ThisDataResultItem.OpenNext = ThisDataResultItem.Last
			ThisDataResultItem.LastPrevious = ThisDataResultItem.Open
			'add the actual log gain item to the list after we have updated the previous bar's OpenNext, so that the list always has the correct values for all bars
			DataResult.Add(ThisDataResultItem)
	...
	Public Function ToCumulativeLogGainInverse(
		...
		Dim ThisDataSourceAtIndex As StockPriceVol = DataSource(Index)
		Dim ThisPriceLogAtIndex As Double = ThisDataSourceAtIndex.Last

		Dim ThisSourceItemLast As StockPriceVol = DataSource.First
		Dim ThisDataResultLast As StockPriceVol = Nothing
		For Each SourceItem In DataSource
			Dim ThisDataResultItem = New StockPriceVol(SourceItem)
			ThisDataResultItem.DataType = StockPriceDataType.RawPrice
			ThisDataResultItem.Last = PriceNormalized * Math.Exp(SourceItem.Last - ThisDataSourceAtIndex.Last)
			...
			MathProcessor.SetHighLow(ThisDataResultItem)
			'If Record.CheckHighLow(ThisDataResultItem.AsIStockPrice, IsApplyCorrection:=False) = False Then
			'	Record.CheckHighLow(ThisDataResultItem.AsIStockPrice, IsApplyCorrection:=True)
			'	ThisDataResultItem = ThisDataResultItem
			'End If
			'Open next is not yet known but by default we can set it to be the same as Last
			'until we see the next bar and can update it, this way we avoid having a gap
			'in the data and we will have a valid value for OpenNext for all bars,
			...
			Else
				...
				'update the previous bar's OpenNext to be the current bar's Open, which is
				'the correct value for the next bar's OpenNext
				ThisDataResultLast.OpenNext = ThisDataResultItem.Open
			End If
			DataResult.Add(ThisDataResultItem)
